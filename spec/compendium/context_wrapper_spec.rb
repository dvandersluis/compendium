...
RSpec.describe Compendium::ContextWrapper do
  describe '.wrap' do
    let(:w1) { Wrapper1.new }
    let(:w2) { Wrapper2.new }
    let(:w3) { Wrapper3.new }
    ...
    it 'does not affect the original objects' do
      subject
      expect(w1).to_not respond_to :wrapped
      expect(w2).to_not respond_to :test_val
    end
    ...
    context 'nested wrapping' do
      ...
      it 'does not extend the inner wrap' do
        subject
        expect(inner).to_not respond_to :wrapper_num
      end
    end
  end
end
